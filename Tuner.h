...
const PROGMEM float Guitar_Note[] =
{
  61.714,  5.385,   69.268,  73.425,  82.410,
...
void guitarTuner()
{
  ...
  tft.fillRoundRect(64,  k, 80, 50, 0, GUI_ITEM_COLOR);
  tft.fillRoundRect(144, k, 32, 50, 0, ILI9341_GREEN);
  tft.fillRoundRect(176, k, 80, 50, 0, GUI_ITEM_COLOR);
  tft.fillRoundRect(256, k, 63, 50, 0, ILI9341_RED);

  tft.setTextColor(ILI9341_WHITE);

  // may halt until a freq is detected
  // note: no obvious way to stop this process once
  // started. Still investigating.
...
printValue("freq", note);
      ...
      tft.fillRoundRect(cents_Graphic - 4, k, 8, 50, 0, GUI_FILL_COLOR);

      // display the note and octave in large font
      if (note_String != last_Note_String)
      {
        tft.fillRect(99, 149, 160, 80, GUI_FILL_COLOR);
